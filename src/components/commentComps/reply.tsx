...
export const Reply = (props: replyProp) => {
  ...
  const postReply = () => {
    ...
    axios
      .post(`http://localhost:8108/replies/${props.parent_id}/`, {
        message: userComment,
        ...
        <div className="w-full flex flex-row justify-between my-2">
          {postingComment && (
            <div className="text-xs text-secondary-content flex flex-row items-center">
              <ProgressSpinner className="h-6 w-6 px-4 ml-24" />
              Posting Comment...
            </div>
          )}
          ...
          <div>
            <Button
              ...
              outlined
              rounded
              label={`${advancedEditor ? "Basic" : "Advanced"}`}
              onClick={() => {
                setAdvancedEditor(!advancedEditor);
              }}
              className="text-s py-0 px-2 mx-2"
            />
            <Button
              disabled={postingComment}
              raised
