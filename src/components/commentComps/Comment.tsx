...
export const Comment = (props: commentProps) => {
  ...
  const showMoreReplies = () => {
    ...
    setLoading(true);
    axios
      .get(`http://localhost:8108/replies/${comment?._id}/`, {
        params: {
          limit: LIMIT,
          skip: offset,
        },
    ...
    <>
      ...
      {replyInputVisible && (
        <div className="relative pl-8 ml-8 mt-4">
          ...
          <Reply
            ...
            setRepliesVisible={setCommentRepliesVisible}
          />
        </div>
      )}
        ...
        <div className="flex flex-col ml-16 mb-4">
          <div>
            <Button
              ...
              raised
              onClick={() => {
                setCommentRepliesVisible(!commentRepliesVisible);
              }}
      ...
      <div className="ml-12">
        {commentRepliesVisible &&
          comment?.replies
            .map((replyId: string) => props.comments.get(replyId)) //gets the comments by id of reply
            .filter((reply): reply is CommentType => !!reply) //removes null values
            .sort(
              (a, b) =>
                new Date(b.created_at).getTime() -
                new Date(a.created_at).getTime(),
            ) // sorting by newest first for now
            .map((reply) => (
              <Comment
      ...
      <div className="flex flex-col ml-16 mb-4">
        {!commentRepliesVisible &&
          (comment?.replies?.length ?? 0) > 0 && ( //if no replies are visible
            <div>
              <Button
                label="View Replies"
                raised
        ...
        {!areAllRepliesVisible() &&
          commentRepliesVisible && ( // if replies are visible but more can be pulled from db
            <div>
              <Button
